...
static void quit(int sig) {
	 ...
	 * really wanted to quit, and if so, checks the high score stuff (with
...
static int tunnel(chtype cmd, int *attribs) {
	/*
	 * tunnel() does the main game work.  Returns 1 if everything's okay, 0
...
void showmoves(bool on, int *attribs) {
	 ...
	 * possible moves instead.  "on" tells showmoves() whether to add or
	 * remove moves.
	 */
	int dy = -1, dx;
...
void lockit(bool on) {
	/*
	 * lockit() creates a file with mode 0 to serve as a lock file.  The
	 * creat() call will fail if the file exists already, since it was made
	 * with mode 0. lockit() will wait approx. 15 seconds for the lock file,
	 * and then override it (shouldn't happen, but might).  "on" says
	 * whether to turn locking on or not.
	 */
	int fd, x = 1;

	if (on) {
		while ((fd = open(LOCKPATH, O_RDWR | O_CREAT | O_EXCL, 0)) <
		       0) {
...
void help(void) {
	/*
	 * help() simply creates a new window over stdscr, and writes the help
	 * info inside it.  Uses macro msg() to save space.
	 */
	if (!helpwin) {
		...
		mvwprintw(
		    helpwin, 1, 2,
		    "Welcome to %s, by Matthew Day <mday@iconsys.uu.net>.",
		    version);
		msg(3, " The object of Greed is to erase as much of the screen "
		       "as");
		msg(4, " possible by moving around in a grid of numbers.  To "
		       "move,");
		msg(5, " use the arrow keys, your number pad, or one of the "
		       "letters");
		mvwprintw(helpwin, 6, 2,
		          " 'hjklyubn'. Your location is signified by the '%c' "
		          "symbol.",
		          ME);
		msg(7, " When you move in a direction, you erase N number of "
		       "grid");
		msg(8, " squares in that direction, N being the first number "
		       "in that");
		msg(9, " direction.  Your score reflects the total number of "
		       "squares");
		msg(10, " eaten.  Greed will not let you make a move that "
		        "would have");
		msg(11, " placed you off the grid or over a previously eaten "
		        "square");
		msg(12, " unless no valid moves exist, in which case your game "
		        "ends.");
		msg(13,
		    " Other Greed commands are 'Ctrl-L' to redraw the screen,");
		msg(14, " 'p' to toggle the highlighting of the possible "
		        "moves, and");
		msg(15,
		    " 'q' to quit.  Command line options to Greed are '-s' to");
